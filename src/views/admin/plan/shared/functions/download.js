import { downloadAnySubmitFileAPi } from '@/api/plan';

export default async function handleDownloadSubmitFile(submitItem) {
  const that = this;
  that.progress.visible = true;
  // 发起请求
  const response = await downloadAnySubmitFileAPi(
    { id: submitItem.id },
    that.progress
  );
  // 处理响应
